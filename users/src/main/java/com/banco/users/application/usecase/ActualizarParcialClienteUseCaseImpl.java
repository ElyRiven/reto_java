    ...
    public Cliente execute(UUID clienteId, PatchClienteCommand command) {
        ...
        try {
            ...
            var payload = new ClienteEvent.Payload(
                    saved.getClienteId(), saved.getNombre(), saved.getEstado(), "PATCH_UPDATE"
            );
            eventProducerPort.publish(new ClienteEvent(metadata, payload));
        } catch (Exception e) {
            log.warn("Error al publicar evento de actualización parcial clienteId={}: {}", clienteId, e.getMessage());
        }
